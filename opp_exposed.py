...
VERY_LATE_CAPTURE_BUFFER = 0  # allow very-late launches for score
...
# Tie-hunt mode: engine gives reward=1 to ALL tied top-scoring players.
TIE_HUNT_RANGE_FLOOR = 0.72
TIE_HUNT_REMAINING_STEPS = 90
TIE_HUNT_SHIP_VALUE = 1.2

# Terminal fleet-in-flight abuse: in-flight fleets count toward final score.
TERMINAL_LAUNCH_TURNS = 8

# Speed-threshold snap thresholds (from log-speed curve):
# 43->3.0, 78->3.5, 137->4.0, 232->4.5, 385->5.0
SPEED_THRESHOLDS = (385, 232, 137, 78, 43)
...
# Exposed-planet detection (from 1224 bot, conservative tuning)
EXPOSED_PLANET_MIN_OUTBOUND = 15
EXPOSED_PLANET_MIN_FLEET_SHIPS = 5
EXPOSED_PLANET_VALUE_MULT = 1.30
EXPOSED_PLANET_MARGIN_RELIEF = 2
    ...
    for planet in enemy_planets:
        outbound = sum(
            ...
            if (
                ...
                and f.ships >= EXPOSED_PLANET_MIN_FLEET_SHIPS
            )
        )
        if outbound >= EXPOSED_PLANET_MIN_OUTBOUND and outbound >= planet.ships:
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        # Per-opponent proximity (Doc-7 feature)
    ...
    def opponent_priority(self, opp_id):
        ...
        norm = (my_dist - min_d) / dist_range
        proximity = 1.25 - 0.7 * norm
        ...
        return max(0.5, proximity + bonus)
    ...
    def probe_ship_candidates(self, src_id, target_id, source_cap, hints=()):
        ...
        # Speed threshold snaps
        for thresh in SPEED_THRESHOLDS:
            if 1 <= thresh <= source_cap:
                values.add(thresh)
    ...
    def best_probe_aim(self, src_id, target_id, source_cap, hints=(),
        ...
        cache_key = (src_id, target_id, max(1, int(source_cap)), tuple(hints),
                     min_turn, max_turn, anchor_turn, max_anchor_diff)
        cache = self.best_probe_cache
    ...
    def min_ships_to_own_by(self, target_id, eval_turn, attacker_owner, arrival_turn=None,
    ...
    def min_ships_to_own_at(self, target_id, arrival_turn, attacker_owner,
                            planned_commitments=None, extra_arrivals=(), upper_bound=None):
    ...
    def reinforcement_needed_to_hold_until(self, planet_id, arrival_turn, hold_until,
...
def build_modes(world):
    ...
    is_tie_hunting = (
        is_behind
        and world.remaining_steps < TIE_HUNT_REMAINING_STEPS
        and world.max_enemy_strength > 0
        and world.my_total >= world.max_enemy_strength * TIE_HUNT_RANGE_FLOOR
    )
    ...
    if is_ffa and not is_finishing:
        attack_margin_mult += 0.05
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if target.owner not in (-1, world.player):
        ...
        value *= base_mult * world.opponent_priority(target.owner)
    ...
    # Exposed enemy (just launched a big fleet) is a soft target
    if target.owner not in (-1, world.player) and target.id in world.exposed_planet_ids:
    ...
    if modes["is_ffa"] and target.owner == -1 and is_safe_neutral(target, policy):
        value *= 1.15

    if modes["is_tie_hunting"] and target.owner != world.player:
        value += max(0, target.ships) * TIE_HUNT_SHIP_VALUE
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if target.owner == -1:
        ...
        # Enemy reinforcement estimate (Doc-7)
        reinforce_est = 0
    ...
    # Exposed planet — garrison is low, less margin needed
    if target.owner not in (-1, world.player) and target.id in world.exposed_planet_ids:
        margin = max(0, margin - EXPOSED_PLANET_MARGIN_RELIEF)

    base = send + margin

    # Speed-threshold snap: bump to next threshold if source can afford it.
    if target.owner != world.player and arrival_turns >= 7:
        for threshold in SPEED_THRESHOLDS:
            if threshold <= base:
                break
            if src_available >= threshold and base >= threshold * 0.55:
                base = threshold
                break

    return min(src_available, base)
...
def settle_plan(src, target, src_cap, send_guess, world, planned_commitments, modes, policy,
    ...
    def evaluate(send):
        ...
        need = world.min_ships_to_own_by(
            ...
            arrival_turn=turns, planned_commitments=planned_commitments, upper_bound=src_cap,
        )
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            plan = settle_reinforce_plan(
                src, target, source_cap, probe,
                world, planned_commitments, hold_until, max_arrival_turn,
            )
            if plan is None:
                continue
            angle, turns, _, need, send_pref = plan
...
def plan_moves(world, deadline=None):
    ...
    def time_filters_pass(target, turns, needed, src_cap):
        if not candidate_time_valid(target, turns, world,
    ...
    for src in world.my_planets:
        ...
        for target in world.planets:
            ...
            if not candidate_time_valid(target, rough_turns, world,
                                         VERY_LATE_CAPTURE_BUFFER if world.is_very_late else LATE_CAPTURE_BUFFER):
                continue
                ...
                if partial_seed is not None:
                    ...
                    if time_filters_pass(target, p_turns, global_needed, src_available):
                        ...
                        if partial_value > 0:
                            ...
                            source_options_by_target[target.id].append(ShotOption(
                                score=partial_score, src_id=src.id, target_id=target.id,
    ...
    for target_id, options in source_options_by_target.items():
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                missions.append(Mission(kind="swarm", score=pair_score, target_id=target_id,
                                          turns=joint_turn, options=[first, second]))

        if (THREE_SOURCE_SWARM_ENABLED and allow_heavy_phase()
            ...
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        missions.append(Mission(kind="swarm", score=trio_score, target_id=target_id,
                                                  turns=joint_turn, options=trio))

        if (FOUR_SOURCE_SWARM_ENABLED and allow_heavy_phase()
                and target.owner not in (-1, world.player)
                ...
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        for l in range(k + 1, len(top_options)):
                            if expired():
                                return finalize_moves()
                            quartet = [top_options[i], top_options[j], top_options[k], top_options[l]]
                            if len({o.src_id for o in quartet}) < 4:
                                continue
                            quart_tol = swarm_eta_tolerance(tuple(quartet), target, world)
                            turns = [o.turns for o in quartet]
                            if max(turns) - min(turns) > quart_tol:
                                continue
                            joint_turn = max(turns)
                            total_cap = sum(o.send_cap for o in quartet)
                            need = world.min_ships_to_own_at(
                                target.id, joint_turn, world.player,
                                planned_commitments=planned_commitments, upper_bound=total_cap,
                            )
                            if need <= 0 or total_cap < need:
                                continue
                            if any(quartet[a].send_cap + quartet[b].send_cap + quartet[c].send_cap >= need
                                    for a in range(4) for b in range(a + 1, 4) for c in range(b + 1, 4)):
                                continue
                            value = target_value(target, joint_turn, "swarm", world, modes, policy)
                            if value <= 0:
                                continue
                            quart_score = apply_score_modifiers(
                                value / (need + joint_turn * ATTACK_COST_TURN_WEIGHT + 1.0),
                                target, "swarm", world,
                            )
                            quart_score *= FOUR_SOURCE_PLAN_PENALTY
                            missions.append(Mission(kind="swarm", score=quart_score, target_id=target_id,
                                                      turns=joint_turn, options=quartet))
    ...
    for mission in missions:
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit"):
            ...
            if mission.kind == "reinforce":
                left = min(source_inventory_left(option.src_id),
            ...
            if mission.kind == "reinforce":
                plan = settle_reinforce_plan(src, target, left, min(left, option.send_cap),
                                              world, planned_commitments, option.anchor_turn, mission.turns)
            elif mission.kind == "rescue":
                plan = settle_plan(src, target, left, min(left, option.send_cap),
                                    world, planned_commitments, modes, policy,
                                    mission="rescue",
        ...
        if missing <= 0 or sum(limits) < missing:
            continue
        ordered = sorted(zip(mission.options, limits),
    ...
    # Follow-up pass with leftover budget
        ...
        for src in world.my_planets:
            ...
            for target in world.planets:
                ...
                if target.id in world.comet_ids and target.production <= LOW_VALUE_COMET_PRODUCTION:
                    continue
                seeded = world.best_probe_aim(src.id, target.id, src_left,
                                               hints=(int(target.ships) + 1,))
                if seeded is None:
                    continue
                _, rough_aim = seeded
                ...
                if send < rough_needed:
                    continue
                plan = settle_plan(src, target, src_left, send,
                                    world, planned_commitments, modes, policy, mission="capture")
                if plan is None:
                    continue
            ...
            if need > src_left:
                continue
            plan = settle_plan(src, target, src_left, min(src_left, send),
                                world, planned_commitments, modes, policy, mission="capture")
            if plan is None:
                continue
            angle, turns, _, need, send = plan
    ...
    # Doomed planet salvage
    if expired():
        return finalize_moves()
    live_doomed = compute_live_doomed()
    if live_doomed:
        frontier_targets = (world.enemy_planets if world.enemy_planets
                             else (world.static_neutral_planets or world.neutral_planets))
        if frontier_targets:
            frontier_distance = {p.id: nearest_distance_to_set(p.x, p.y, frontier_targets)
        ...
        for planet in world.my_planets:
            ...
            for target in world.planets:
                ...
                if target.id == planet.id or target.owner == world.player:
                    continue
                seeded = world.best_probe_aim(planet.id, target.id, available_now,
                ...
                if need <= 0 or need > available_now:
                    continue
                plan = settle_plan(planet, target, available_now,
            ...
            if not safe_allies:
                continue
            retreat_target = min(safe_allies,
                                  key=lambda ally: (frontier_distance.get(ally.id, 10 ** 9),
    ...
    if ((world.enemy_planets or world.neutral_planets) and len(world.my_planets) > 1
            and not world.is_late and allow_optional_phase()):
        live_doomed = compute_live_doomed()
        frontier_targets = (world.enemy_planets if world.enemy_planets
                             else (world.static_neutral_planets or world.neutral_planets))
        frontier_distance = {p.id: nearest_distance_to_set(p.x, p.y, frontier_targets)
        ...
        if safe_fronts:
            ...
            send_ratio = (REAR_SEND_RATIO_FOUR_PLAYER if world.is_four_player
            ...
            for rear in sorted(world.my_planets, key=lambda p: -frontier_distance[p.id]):
                ...
                if frontier_distance[rear.id] < frontier_distance[front_anchor.id] * REAR_DISTANCE_RATIO:
                    continue
                stage_candidates = [p for p in safe_fronts
                                     if p.id != rear.id
    ...
    # Total-war endgame: dump at weakest enemy
    if world.is_total_war and world.enemy_planets and allow_optional_phase():
        weakest_enemy = min(world.enemy_planets,
                             key=lambda e: world.owner_strength.get(e.owner, 0))
        for src in world.my_planets:
            if expired():
                return finalize_moves()
            ...
            if atk_left < 5:
                continue
            aim = world.plan_shot(src.id, weakest_enemy.id, atk_left)
            if aim is None:
                target_planets = sorted(world.enemy_planets,
                                          key=lambda e: planet_distance(src, e))
                for ep in target_planets:
    ...
    # TERMINAL fleet-launch phase (new improvement):
    # In the final TERMINAL_LAUNCH_TURNS, launch every spare ship toward the
    # nearest target. In-flight fleets count toward final score, and since
    # the game ends before most arrive, this is pure score-padding.
    if world.remaining_steps <= TERMINAL_LAUNCH_TURNS and world.remaining_steps >= 1:
        for src in world.my_planets:
            if expired():
                return finalize_moves()
            spare = source_inventory_left(src.id)
            if spare < 2:
                continue
            candidates = (world.enemy_planets or world.neutral_planets
                          or [p for p in world.my_planets if p.id != src.id])
            if not candidates:
                continue
            best_tgt = min(candidates, key=lambda t: planet_distance(src, t))
            aim = world.plan_shot(src.id, best_tgt.id, spare)
            if aim is None:
                continue
            angle, _, _, _ = aim
            append_move(src.id, angle, spare)
