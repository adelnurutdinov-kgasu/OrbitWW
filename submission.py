...
FINISHING_HOSTILE_VALUE_MULT = 1.2
BEHIND_ROTATING_NEUTRAL_VALUE_MULT = 0.92
EXPOSED_PLANET_VALUE_MULT = 1.75
...
PROACTIVE_DEFENSE_RATIO = 0.20
MULTI_ENEMY_PROACTIVE_HORIZON = 14
MULTI_ENEMY_PROACTIVE_RATIO = 0.24
...
def plan_moves(world, deadline=None):
    ...
    # Total-war endgame: dump all remaining attack budget at the weakest enemy
    if world.is_total_war and world.enemy_planets and allow_optional_phase():
        weakest_enemy = min(world.enemy_planets, key=lambda e: world.owner_strength.get(e.owner, 0))
        for src in world.my_planets:
            if expired():
                return finalize_moves()
