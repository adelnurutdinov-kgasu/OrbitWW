...
def _t_solo_capture(state, att, tgt, incoming=0.0):
    ...
    for t_wait in range(0, ATTACK_HORIZON + 1):
        ...
        t_arrive = t_wait + eta
        defender = tgt.ships + eff_prod_def * max(0.0, t_arrive - proj_at)
...
def _required_strike(state, att, tgt, omega, init_eta_ships, incoming):
    ...
    needed = 0
    eff_prod = _effective_prod(tgt)
    overkill = _overkill(tgt)
    for _ in range(ETA_REFINE_ITERS + 1):
        # производство только ПОСЛЕ projected_at (до него уже учтено в tgt.ships)
        # и ТОЛЬКО для owned планет (нейтрал не производит — см. orbit_sim).
        prod_dt = max(0.0, eta - proj_at)
        defender = tgt.ships + eff_prod * prod_dt
...
def eval_multi_sync(state, sup, att, tgt, incoming=0.0):
    ...
    proj_at_tgt = _projected_at(state, tgt.id)
    prod_dt = max(0.0, t_sync - proj_at_tgt)
    defender  = tgt.ships + _effective_prod(tgt) * prod_dt
    effective = max(0.0, defender - incoming)
...
def eval_pipe(state, sup, att, tgt, incoming=0.0):
    ...
    proj_at_tgt = _projected_at(state, tgt.id)
    prod_dt     = max(0.0, t_total - proj_at_tgt)
    defender    = tgt.ships + _effective_prod(tgt) * prod_dt
    effective   = max(0.0, defender - incoming)
    needed      = int(math.ceil(effective)) + _overkill(tgt)
